                    ...
                    <div className="relative z-50 sm:max-w-sm sm:w-full m-3 sm:mx-auto">
                        <div className="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto ">
                            <div className="flex justify-between items-center py-3 px-4 border-b">
                                ...
                                <button type="button" className="flex justify-center items-center size-7 text-sm font-semibold rounded-full border border-transparent text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none" onClick={handleClose}>
                            ...
                            <div className="p-4 overflow-y-auto">
                                ...
                                <p className="mx-1 mt-4 pt-2 text-gray-500 text-center border-t">共有しているユーザー</p>
                                {list && list.guests_info ? (
                                    list.guests_info.length > 0 ? (
                                        list.guests_info.map((guest, index) => (
                                            <div key={guest.member_id} className="flex-row ml-4 text-gray-800 flex">
                                        ...
                                        <p className="ml-4 text-gray-800">共有しているユーザーはいません。</p>
                                    )
                                ) : (
                                    <p className="ml-4 text-gray-800">　</p>
                                )}
                            </div>
                            <div className="flex justify-center items-center gap-x-2 py-3 px-4 border-t">
                                <button type="button" onClick={() => handleDeleteList(list.list_id)} className="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-red-600 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none">
                                    このリストを削除する
                                </button>
                            </div>
                        </div>
